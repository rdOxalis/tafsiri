...
### Changed
- **The Linux `.tar.gz` no longer carries the build machine's home directory either** (ADR-069). The fix below reached the `.deb` and stopped there, because the tarball was made by hand outside the packaging script and so never saw it. `build_deb.sh` now produces both artefacts, both by default and through the same step, which is what stops the next one being forgotten rather than another round of remembering. Cosmetic, as before: nothing about it ever affected how the app runs.
...
### Changed
- **The two language fields are called "Learning language" and "Confident language"** (ADR-059), in all 12 UI languages. They were "Primary" and "Secondary", and testers kept filling them in the wrong way round — reasonably, because a "primary language" sounds like your own, while the app means the one you are learning and translating *into*. In Swahili it was not merely vague but backwards: *lugha ya msingi* is how a mother tongue is described, and *lugha ya pili* is the ordinary term for a language one has learnt, so both labels said the opposite of what the field does. Bulgarian had the same inversion, and every other language leaned that way. The explanations behind the ⓘ now also say which direction the translation runs, instead of leaving that to the label. Your settings are untouched: only the wording changed, not what is stored.
