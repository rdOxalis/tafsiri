...
**Script detection closes the gap the settings cannot** (added the same day). Deriving the OCR language from the configured *target* languages is backwards: a translation app is used on text one cannot read, so the script in the image may be one the user never configured — primary German, secondary English, photograph a Cyrillic sign, and the read fails with nothing useful to say. Worse, both language fields are deliberately free text (the AI accepts any spelling in any script), so the name→code lookup can miss for reasons that have nothing to do with the image. Both are answered by asking the image instead of the settings: after a rejected read, `tesseract --psm 0` reports the script using `osd.traineddata`, which ships with the engine. If that script was not among the loaded data, the recognition is retried with Tesseract's **script-level** trained data (`script/Cyrillic`, one package per script covering every language written in it), and only if that is not installed does the user get told what to install — `tesseract-ocr-script-cyrl`. The retry runs only after a failure, so the ordinary path is unchanged. A Latin result is exempt from the advice: Latin is already covered by the loaded Latin languages, so a failure there means a bad photograph and an install hint would misdirect. This demotes the free-text problem from an outright failure to a loss of accuracy — within one script, picking `eng` instead of `deu` costs diacritics, which the model downstream restores. **Limit:** OSD needs enough characters; measured, four lines of Cyrillic are detected and a two-word sign returns "Too few characters", falling back to the plain message. It improves the outcome, it does not guarantee it.
