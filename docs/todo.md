...
- [ ] **The macOS pass.** Entitlements and usage descriptions are prepared but **unverified — nobody has built this app for macOS** (ADR-048). Expect in this order: it should now translate (the sandbox blocked every outgoing call before) and the microphone should prompt rather than terminate the app; then the real question, **image-to-text under the App Sandbox**. Recognition shells out to `tesseract`, and a sandboxed child may not be able to reach `/opt/homebrew/bin`. If that bites, take the sandbox off: it is only needed for the Mac App Store, which was never the plan, and direct distribution asks for notarization and the Hardened Runtime instead — neither of which blocks spawning a binary. Bundling is decided against (ADR-050), so this is a one-line entitlement change rather than a packaging project. Clipboard image paste is not implemented for macOS either; `osascript -e 'the clipboard as «class PNGf»'` is the route, behind the same `ClipboardImageService` seam. `brew install tesseract tesseract-lang` covers the engine and every language.
