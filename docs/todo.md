...
- [ ] **(Android) Clipboard images for paste.** Ctrl+V / the paste button read images on Linux only (ADR-040). Android carries images on the clipboard as `content://` URIs, which needs a small platform channel into `ClipboardManager` — the `ClipboardImageService` interface is already the seam. Worth doing: on a phone a screenshot is the most likely way an image arrives at all.
- [ ] **(Windows) Clipboard images for paste.** Same interface, but pointless until Tesseract is bundled (ADR-037) — a pasted image would have nothing to read it. Do the two together. A PowerShell round-trip through `[Windows.Forms.Clipboard]::GetImage()` is the dependency-free option. (ADR-040)

- [ ] **Close F-Droid MR #39249** (needs your GitLab login — one click): https://gitlab.com/fdroid/fdroiddata/-/merge_requests/39249 (ADR-030)

- [ ] **Run the Windows build on an actual Windows machine.** Everything was authored on Linux: `.\build_windows.ps1` must be executed with the Flutter SDK, Visual Studio 2022 ("Desktop development with C++") and Inno Setup 6.3+ present. The CI job has never run either — the `Release` workflow was added in `79d2112` (2026-08-14), after the newest tag `v1.0.10` (2026-08-12), so no Windows build of this app has ever been produced anywhere. Then check, in order: the app starts, the title bar and taskbar show the Tafsiri name and icon, a translation is saved and reappears in the history (that is the bundled `sqlite3.dll` working), backup save/restore opens the Windows file dialogs, and finally install → Start menu → uninstall including the keep-your-data prompt. (ADR-035)

  Newer behaviour worth checking in the same pass: **the microphone should be visible and should work** — `speech_to_text_windows` is beta and has never been exercised, and it is the one thing here with a real chance of misbehaving (ADR-039). The image button is expected to report "install Tesseract" (ADR-037). **Ctrl+V must still paste text**; pasting an image is Linux-only so far (ADR-040). Both language lists should be alphabetical and the two language fields should carry info buttons (ADR-039).
...
- [x] Settings polish: microphone hidden on desktop rather than greyed out, both language pickers sorted alphabetically (case- and accent-insensitive, non-Latin grouped last), info buttons on the primary/secondary language fields explaining what each decides. 8 new tests. (2026-08-14, ADR-039)
