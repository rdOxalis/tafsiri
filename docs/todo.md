...
- [ ] **Finish the macOS pass.** It builds and runs (2026-08-15). Getting there took two workarounds worth knowing: `speech_to_text` ships a `Package.swift` declaring `.macOS("10.14")` while its own podspec says `11.00` and its Swift needs 10.15, so Swift Package Manager cannot compile it — `flutter config --no-enable-swift-package-manager` makes CocoaPods read the correct manifest, and **that is a per-machine setting, not a repository one**. Worth reporting upstream. The App Sandbox is now off (ADR-052) because it refused to launch Tesseract. Still unchecked: **translation against a real API key**, **the microphone** — currently greyed out, and the `[STT] init failed` line in the debug console will say why — **image-to-text** now that the sandbox is gone, the history surviving a restart, and the backup dialogs. Clipboard image paste is not implemented for macOS; `osascript -e 'the clipboard as «class PNGf»'` is the route, behind the same `ClipboardImageService` seam.
