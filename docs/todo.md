...
- [ ] **The macOS pass.** First build attempted 2026-08-15 and it does not compile yet — **not because of anything in this project**: `speech_to_text` 7.3.0 *and* 7.4.0 ship a `Package.swift` declaring `.macOS("10.14")` while their own podspec says `11.00`, and the plugin's Swift uses `SFTranscription`, which needs 10.15. Under Swift Package Manager the wrong manifest wins and the build fails on availability. Workaround: `flutter config --no-enable-swift-package-manager`, then `flutter clean` and build again — CocoaPods reads the podspec and compiles. **That is a per-machine Flutter setting, not a repo one**, so it has to be set on every Mac that builds this. Worth reporting upstream; the two manifests simply disagree. Once it builds, the order to check is: translation works (the sandbox blocked every outgoing call before ADR-048), the microphone prompts rather than terminating the app, then image-to-text — `brew install tesseract tesseract-lang`, and if a sandboxed child cannot reach `/opt/homebrew/bin`, take the sandbox off; it is only needed for the Mac App Store, which was never the plan, and bundling is decided against (ADR-050). Clipboard image paste is not implemented for macOS; `osascript -e 'the clipboard as «class PNGf»'` is the route, behind the same `ClipboardImageService` seam.
