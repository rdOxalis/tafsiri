...
- [ ] **The macOS pass.** Entitlements and usage descriptions are prepared but **unverified — nobody has built this app for macOS** (ADR-048). Expect in this order: it should now translate (the sandbox blocked every outgoing call before) and the microphone should prompt rather than terminate the app; then the real question, **image-to-text under the App Sandbox**. Recognition shells out to `tesseract`, and a sandboxed child cannot reach `/opt/homebrew/bin` or read the image outside the container — so either the sandbox comes off (fine for direct distribution, rules out the Mac App Store) or Tesseract gets bundled into the app, which is the same packaging question already open for Windows. Clipboard image paste is not implemented for macOS either; `osascript -e 'the clipboard as «class PNGf»'` is the route, behind the same `ClipboardImageService` seam. `brew install tesseract tesseract-lang` covers the engine and every language.
