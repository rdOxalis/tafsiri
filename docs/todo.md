...
- [ ] **(Android) Cyrillic — and every non-Latin script — reads as nonsense.** Bulgarian has been a UI language since 1.0.11, but ML Kit's on-device recognisers cover only Latin, Chinese, Devanagari, Japanese and Korean; there is no Cyrillic model, and `TextRecognizer()` without a script argument is the Latin one. Photograph a Bulgarian sign on a phone and it returns the same confident Latin transliteration (`Моля те` → `Mona Te`) that ADR-038 spent a night removing on the desktop — and without a confidence gate, because ML Kit reports no per-word confidence to gate on. Nothing is broken that used to work; it has simply never been possible there. Two routes, and they are not alternatives so much as stages:

  **Now, cheap and honest:** when a *configured* language is written in a script ML Kit cannot read, refuse rather than return junk — the same shape as `OcrLanguageMissingException` on the desktop. It does not make Bulgarian readable, but it stops the app lying. Partial by nature: the case that actually bit on the desktop was photographing Cyrillic while configured for Swahili and German, and ML Kit offers no script detection to catch that.

  **Properly:** Tesseract through the NDK, which is the long-standing "(Later) Replace ML Kit with Tesseract on Android" item below and would settle three things at once — every script readable, the per-word confidence the gate needs, and the proprietary blob gone, which is what blocks official F-Droid (ADR-028, ADR-030). `flutter_tesseract_ocr` (0.4.31, June 2026, android/ios/web) is the obvious candidate; `tesseract_ocr` (0.5.0) the other. Both bundle tessdata as assets, so the size question from ADR-050 returns — but in the opposite direction, since dropping ML Kit removes most of the 83 MB APK. The `OcrService` interface is already the seam.
...
### Done
- [x] Debian package: plugin libraries no longer carry the build machine's home directory as RUNPATH — rewritten to `$ORIGIN` at packaging time. Verified harmless before and after with the build tree hidden; fixed for hygiene, not function. `patchelf` is now required to build the .deb. The 1.0.17 release asset was replaced with the cleaned package the same day. (2026-09-05, ADR-066)
